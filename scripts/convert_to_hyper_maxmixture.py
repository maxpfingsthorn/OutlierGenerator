...
def output_batch(f,batch,dim,null_weight,null_inf_factor):
	...
	f.write( edge_tag+"_MIXTURE ")

	f.write( batch['reference'] +' ' + batch['hyper_constraints'][0]['target']+' ')
	...
	if batch['has_null_hypothesis']:
		constraint=batch['hyper_constraints'][0]['constraints'][0]
		scaled_inf = [ str(null_inf_factor * float(x)) for x in constraint[(3+mean_len+1):] ]
		f.write(" %s %s %s %s %s %s" % ( edge_tag, str(null_weight), batch['reference'], batch['hyper_constraints'][0]['target'], " ".join(constraint[3:(3+mean_len)]), " ".join(scaled_inf) ) )

	for c in batch['hyper_constraints']:

		for m in range(0,len(c['constraints'])):

			f.write( " %s %s %s %s %s" %( edge_tag, str( (c['hyper_weight']/norm_fac) * (c['weights'][m]/sum(c['weights'])) ), batch['reference'], c['target'], " ".join(c['constraints'][m][3:]) ))
